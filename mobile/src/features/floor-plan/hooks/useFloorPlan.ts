...
type AddRoomExecutor = {
  execute: (userId: string, input: CreateRoomInput) => Promise<Room>;
};
...
type UpdateFurnitureVariables = {
  furnitureId: string;
  input: UpdateFurnitureInput;
};
type DeleteFurnitureExecutor = {
  execute: (userId: string, furnitureId: string) => Promise<void>;
};

export function buildFloorPlanQuery(
  userId: string,
  repository: Pick<FloorPlanRepository, "getFloorPlan">,
) {
  return {
...
export function buildAddRoomMutationOptions(
  ...
  return {
    ...
    onMutate: async (input: CreateRoomInput) => {
      ...
      const previous = queryClient.getQueryData<FloorPlan>([
        "floorPlan",
        userId,
      ]);
...
export function useFloorPlan(userId: string, repository: FloorPlanRepository) {
  ...
  const addFurniture = useMutation(
    buildAddFurnitureMutationOptions(
      queryClient,
      userId,
      new AddFurnitureUseCase(repository),
    ),
  );
  ...
  const deleteRoom = useMutation({
    mutationFn: (roomId: string) =>
      new DeleteRoomUseCase(repository).execute(userId, roomId),
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["floorPlan", userId] });
    },
  });
