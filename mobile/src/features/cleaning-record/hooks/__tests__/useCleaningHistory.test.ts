...
import type { CleaningRecord, ListRecordsParams, UpdateRecordInput } from '../../types';
import {
...
describe('useCleaningHistory', () => {
    ...
    describe('正常系: 履歴リストを取得して返す', () => {
        it('returns_records_list_when_history_is_fetched', async () => {
            ...
            const query = buildCleaningHistoryQuery('user-1', {}, mockRepository as never);
    ...
    describe('正常系: partId 指定時にフィルタリングパラメータが渡る', () => {
        it('passes_partId_filter_param_when_partId_is_specified', async () => {
            ...
            const query = buildCleaningHistoryQuery('user-1', params, mockRepository as never);
            const result = await queryClient.fetchQuery(query);

            expect(mockRepository.listRecords).toHaveBeenCalledWith('user-1', { partId: 'part-1' });
    ...
    describe('正常系: queryKey に userId と partId が含まれる', () => {
        it('includes_partId_in_queryKey_when_specified', () => {
            ...
            expect(query.queryKey).toEqual(['cleaning-records', { userId: 'user-1', partId: 'part-1' }]);
        });

        it('excludes_partId_from_queryKey_when_not_specified', () => {
            const query = buildCleaningHistoryQuery('user-1', {}, mockRepository as never);

            expect(query.queryKey).toEqual(['cleaning-records', { userId: 'user-1' }]);
        });
    });
    ...
    describe('正常系: 記録削除後にクエリが invalidate される', () => {
        it('invalidates_cleaning_records_query_after_deleting_record', async () => {
            ...
            queryClient.setQueryData(['cleaning-records', { userId: 'user-1' }], [mockRecord1, mockRecord2]);
            const invalidateSpy = jest.spyOn(queryClient, 'invalidateQueries');

            const options = buildDeleteRecordMutationOptions(queryClient, 'user-1', mockRepository as never);
    ...
    describe('正常系: 記録更新後にクエリが invalidate される', () => {
        it('invalidates_cleaning_records_query_after_updating_record', async () => {
            ...
            const options = buildUpdateRecordMutationOptions(queryClient, 'user-1', mockRepository as never);
            await options.mutationFn({ recordId: 'record-1', input: { note: '更新済み' } });
            options.onSettled!();

            expect(mockRepository.updateRecord).toHaveBeenCalledWith('user-1', 'record-1', { note: '更新済み' });
            expect(invalidateSpy).toHaveBeenCalledWith({
                queryKey: ['cleaning-records', { userId: 'user-1' }],
            });
        });
