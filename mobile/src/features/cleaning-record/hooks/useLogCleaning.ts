...
type LogCleaningExecutor = { execute: (userId: string, input: CreateRecordInput) => Promise<CleaningRecord[]> };
...
export function buildLogCleaningMutationOptions(
    ...
    return {
        ...
        onMutate: async (input: CreateRecordInput) => {
            await queryClient.cancelQueries({ queryKey: ['cleaning-records', { userId }] });
            ...
            const optimisticRecords: CleaningRecord[] = input.partIds.map((partId, index) => ({
...
export function useLogCleaning(userId: string, repository: CleaningRecordRepository) {
