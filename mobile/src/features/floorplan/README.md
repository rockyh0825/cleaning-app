# floorplan-editor feature

## 概要

`features/floorplan` は間取りエディタ機能を担う feature モジュールです。ユーザーが部屋（Room）・家具（Furniture）を配置し、フロアプランを視覚的に編集できます。
...
- `features/floorplan` から他の feature への import は禁止（boundaries ルール）
