...
- [x] 7. presentation層: RoomController / FurnitureController / FloorMapController ✅ 実装済み
  - File: backend/.../floorplan/presentation/{RoomController,FurnitureController,FloorMapController}.kt
...
- [x] 12. features/floorplan/hooks: useFloormap（TanStack Query統合）
  - File: mobile/src/features/floorplan/hooks/useFloormap.ts
  - useQueryで取得、useMutationで楽観的更新、保存失敗時のロールバック
  - Purpose: UIとユースケースの橋渡し・サーバー状態管理
  - **Red**: `hooks/__tests__/useFloormap.test.ts` を先に作成し `npx jest src/features/floorplan/hooks` で失敗を確認
